// Ported from chiguru-owner-web's src/lib/i18n-data.ts (same production
// translations) - only the storage/TTS calls below are RN-specific.
...
export async function readStoredCountry(): Promise<string | null> {
  try {
    const stored = await AsyncStorage.getItem(COUNTRY_STORAGE_KEY);
...
export async function storeCountry(code: string | null) {
  try {
    if (code) await AsyncStorage.setItem(COUNTRY_STORAGE_KEY, code);
    else await AsyncStorage.removeItem(COUNTRY_STORAGE_KEY);
  } catch {
    /* ignore */
  }
}
...
/** Read text aloud in the current language using the device's speech engine. */
export function speak(text: string, lang: Lang) {
  try {
    Speech.stop();
    Speech.speak(text, { language: LOCALE[lang], rate: 0.95 });
  } catch {
